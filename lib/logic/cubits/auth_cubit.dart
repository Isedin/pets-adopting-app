...
class AuthCubit extends Cubit<AuthState> {
  ...
  AuthCubit({FirebaseAuth? auth})
    ...
    _sub = _auth.authStateChanges().listen(
      (u) => emit(
        state.copyWith(user: u, loading: false, error: null, message: null),
      ),
      onError: (e) =>
          emit(state.copyWith(user: null, loading: false, error: '$e')),
    );
  }
  ...
  Future<void> sendPasswordReset(String email) async {
    ...
    try {
      ...
      emit(
        state.copyWith(loading: false, message: 'Password reset email sent.'),
      );
    } on FirebaseAuthException catch (e) {
      emit(state.copyWith(loading: false, error: _friendlyError(e)));
    } catch (e) {
